...
const API_BASE_URL = (
  process.env.UTM_API_URL || "https://generador-utm-fava.vercel.app"
).replace(/\/$/, "");
...
function checkConfig(): { ok: boolean; error?: string } {
  if (!API_KEY) {
    return {
      ok: false,
      error:
        "Falta configurar la variable UTM_API_KEY en el entorno del servidor MCP. Esta clave debe coincidir con la API_SECRET_KEY configurada en Vercel.",
    };
  }
  return { ok: true };
}
...
server.tool(
  "get_taxonomies_and_links",
  "Obtiene las campañas oficiales, fuentes, medios, tags y links genéricos guardados (como Tienda Fava) de la base de datos de Fava. Permite a la IA conocer los nombres exactos y URLs base antes de generar UTMs.",
  {},
  async () => {
...
server.tool(
  "generate_utm_link",
  "Crea un enlace con parámetros UTM oficiales, opcionalmente genera enlace corto Bitly y código QR, y lo guarda en la base de datos de Fava para que aparezca en el sistema web. Si no se provee originalUrl, busca automáticamente en los links genéricos usando linkName (ej: 'Tienda Fava').",
  {
...
server.tool(
  "list_utm_links",
  "Consulta los enlaces UTM guardados en la base de datos de Fava para verificar si ya existen o revisar campañas activas.",
  {
